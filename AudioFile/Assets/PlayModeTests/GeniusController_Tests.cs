...
{ 
    public class GeniusController_Tests
    {
        ...
        public IEnumerator GetGeniusTrackUrlAsync_ValidQuery_ReturnsResults()
        {
            ...
            //Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("Mf doom", "doomsday");
            //Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("mf doom", "doomsday");
            //Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("MF DOOM", "DOOMSDAY"); //All caps when you spell the man's name :)
            //Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("not DooM", "DoOmSdAy"); //Will give you a result, but it's Genius's best guess which is not Doom
            Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("Mf DooM", "DoOmSdAy"); //All caps when you spell the man's name :)
        ...
        public IEnumerator GetGeniusTrackUrlAsync_ValidTrickyQueries_ReturnsResults()
        {
            ...
            Task<List<string>> searchTask = geniusController.FetchGeniusTrackUrlAndIDAsync("MF DOOM", "Rock Co.Kane Flow ft. De La Soul");
            Task<List<string>> searchTask2 = geniusController.FetchGeniusTrackUrlAndIDAsync("MF DOOM", "?");
            Task<List<string>> searchTask3 = geniusController.FetchGeniusTrackUrlAndIDAsync("Kendrick Lamar", "untitled 01 | 08.19.2014.");
        ...
        [UnityTest]
        public IEnumerator FetchGeniusTrackSampleInfoAsync_HasSamplesAndSampledBy_ReturnsResults()
        {
            var geniusController = GeniusWebClientController.Instance;
            geniusController.Start();
            // Wait for the Start method to complete
            ...
            // Act: Run the search
            Task<List<string>> searchTask = geniusController.FetchGeniusTrackSampleInfoAsync("217266"); //This if for Drake Worst Behavior
            ...
            string trackSamples = results[0];
            string sampledBys = results[1];

            // Assert: Ensure we got results
            Assert.IsNotNull(searchTask.Result);
            Debug.Log($"Search Results: {searchTask.Result}");
            Assert.IsNotEmpty(searchTask.Result);
            Assert.IsTrue(trackSamples.Contains("Mo' Money Mo' Problems by The Notorious B.I.G. (Ft. Diddy & Ma$e)"));
            Assert.IsTrue(sampledBys.Contains("#Cinemafridays by Skabzemortal, Worst Behavior Freestyle (Live) by Blackwood DMV, Wave (Album Version) by Bluvercetti (Ft. Pmz), Sozinho no Sukyia by Raffa Moreira (Ft. Makalister), Hold My Phone #SoGoneChallenge by Soos"));
        }

        [UnityTest]
        public IEnumerator FetchGeniusTrackSampleInfoAsync_HasNoSamplesAndSampledBy_ReturnsResults()
        {
            var geniusController = GeniusWebClientController.Instance;
            geniusController.Start();
            // Wait for the Start method to complete
            ...
            // Act: Run the search
            Task<List<string>> searchTask = geniusController.FetchGeniusTrackSampleInfoAsync("2185328"); //This if for Beethoven's Moonlight Sonata
            ...
            Debug.Log($"Search Results: {searchTask.Result}");
            Assert.IsNotEmpty(searchTask.Result);
            Assert.IsTrue(string.IsNullOrEmpty(trackSamples));
            Assert.IsTrue(string.IsNullOrEmpty(sampledBys));
        }
